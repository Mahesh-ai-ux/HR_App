...
#  FACE RECOGNITION
# ══════════════════════════════════════════════════════
@app.post("/api/auth/face-login", response_model=schemas.Token)
def face_login(data: schemas.FaceLoginRequest, db: Session = Depends(get_db)):
    """Match incoming 128-d face descriptor against all stored descriptors."""
    import json as _json, math as _math
    users = db.query(models.User).filter(models.User.face_descriptor.isnot(None)).all()
    if not users:
        raise HTTPException(404, "No face data registered. Please login with email first, then enroll your face from My Profile.")
    best_user, best_dist = None, float("inf")
    for u in users:
        try:
            stored = _json.loads(u.face_descriptor)
            dist = _math.sqrt(sum((a - b) ** 2 for a, b in zip(data.descriptor, stored)))
            if dist < best_dist:
                best_dist = dist
                best_user = u
        except Exception:
    ...
    if best_user and best_dist < THRESHOLD:
        token_str = auth.create_access_token({"sub": str(best_user.id), "role": best_user.role})
        return {"access_token": token_str, "token_type": "bearer",
                "role": best_user.role, "name": best_user.name, "user_id": best_user.id}
    raise HTTPException(401, f"Face not recognized. Please try again or use email login.")


@app.put("/api/employees/me/face")
def save_face_descriptor(data: schemas.FaceDescriptorSave,
                          db: Session = Depends(get_db),
                          me=Depends(auth.get_current_user)):
    """Save face descriptor for the current user."""
    import json as _json
    if len(data.descriptor) != 128:
        raise HTTPException(400, "Invalid face descriptor (must be 128 floats)")
    me.face_descriptor = _json.dumps(data.descriptor)
    db.commit()
    return {"message": "Face enrolled successfully"}


@app.get("/api/employees/me/face-status")
def face_status(db: Session = Depends(get_db), me=Depends(auth.get_current_user)):
    return {"enrolled": me.face_descriptor is not None}
